// Adapté de Qindu, paquet internal/pii — fichier engine.go (origine
// AGPL-3.0 ; reprise approuvée par le titulaire des droits, pont de
// licence suivi au registre des risques R-006). La phase dédiée au
// détecteur NAME (dépendant d'EMAIL) est retirée avec les détecteurs de
// données personnelles — voir le commentaire du paquet (secrets.go).
...
func (e *Engine) Detect(text string) ([]Entity, error) {
	...
	// Recognizers are stateless; sequential execution is safe.
	var allEntities []Entity
	for _, r := range recognizers {
		allEntities = append(allEntities, r.Detect(text)...)
	}
